...
var rate = function (session, imageId, email, anger, disgust, fear, joy, sadness, surprise) {
  ...
  ['MATCH (u1:User)-[r1:RATED]->(p:Image)<-[r2:RATED]-(u2:User)',
    ...
    'SQRT( REDUCE(x=0.001, a IN COLLECT(r1.anger) | x + a^2) ) AS anger_r1_length,',
    'SQRT( REDUCE(y=0.001, b IN COLLECT(r2.anger) | y + b^2) ) AS anger_r2_length,',
    '// disgust',
    'SUM(r1.disgust*r2.disgust) as disgust_dot_product,',
    'SQRT( REDUCE(x=0.001, a IN COLLECT(r1.disgust) | x + a^2) ) AS disgust_r1_length,',
    'SQRT( REDUCE(y=0.001, b IN COLLECT(r2.disgust) | y + b^2) ) AS disgust_r2_length,',
    '// fear',
    'SUM(r1.fear*r2.fear) as fear_dot_product,',
    'SQRT( REDUCE(x=0.001, a IN COLLECT(r1.fear) | x + a^2) ) AS fear_r1_length,',
    'SQRT( REDUCE(y=0.001, b IN COLLECT(r2.fear) | y + b^2) ) AS fear_r2_length,',
    '// joy',
    'SUM(r1.joy*r2.joy) as joy_dot_product,',
    'SQRT( REDUCE(x=0.001, a IN COLLECT(r1.joy) | x + a^2) ) AS joy_r1_length,',
    'SQRT( REDUCE(y=0.001, b IN COLLECT(r2.joy) | y + b^2) ) AS joy_r2_length,',
    '// sadness',
    'SUM(r1.sadness*r2.sadness) as sadness_dot_product,',
    'SQRT( REDUCE(x=0.001, a IN COLLECT(r1.sadness) | x + a^2) ) AS sadness_r1_length,',
    'SQRT( REDUCE(y=0.001, b IN COLLECT(r2.sadness) | y + b^2) ) AS sadness_r2_length,',
    '// surprise',
    'SUM(r1.surprise*r2.surprise) as surprise_dot_product,',
    'SQRT( REDUCE(x=0.001, a IN COLLECT(r2.surprise) | x + a^2) ) AS surprise_r1_length,',
    'SQRT( REDUCE(y=0.001, b IN COLLECT(r2.surprise) | y + b^2) ) AS surprise_r2_length',
    ...
    'SET s.surprise = surprise_dot_product / (surprise_r1_length * surprise_r2_length)',
    'SET s.rating = (s.disgust + s.fear + s.joy + s.sadness + s.surprise+ s.anger)/7'].join('\n'),{}
    );
};
